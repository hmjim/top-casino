...
// if ( $_SERVER["HTTP_HOST"] != "topzz-cassinozz.azurewebsites.net" ) {
...
if($_SERVER['REQUEST_URI'] == '/casino'){
		...
		header('Location: https://topzz-cassinozz.azurewebsites.net'); 
		exit;	
}
if($_SERVER['REQUEST_URI'] == '/news'){
		header('HTTP/1.1 301 Moved Permanently');
		header('Location: https://topzz-cassinozz.azurewebsites.net/news/'); 
		exit;	
}
if($_SERVER['REQUEST_URI'] == '/slots'){
		header('HTTP/1.1 301 Moved Permanently');
		header('Location: https://topzz-cassinozz.azurewebsites.net/slots/'); 
		exit;	
}
...
curl_close( $ch );
$bodytag = str_replace( "((?!topp-cazino\.net/b)\w+(?:\.\w+)+", "https://topzz-cassinozz.azurewebsites.net", $contents );
$result  = preg_replace( '~' . $new_url . '~m', "topzz-cassinozz.azurewebsites.net", $contents );
