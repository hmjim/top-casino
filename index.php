...
// if ( $_SERVER["HTTP_HOST"] != "topz-cazinos.azurewebsites.net" ) {
...
if($_SERVER['REQUEST_URI'] == '/casino'){
		...
		header('Location: https://topz-cazinos.azurewebsites.net'); 
		exit;	
}
if($_SERVER['REQUEST_URI'] == '/news'){
		header('HTTP/1.1 301 Moved Permanently');
		header('Location: https://topz-cazinos.azurewebsites.net/news/'); 
		exit;	
}
if($_SERVER['REQUEST_URI'] == '/slots'){
		header('HTTP/1.1 301 Moved Permanently');
		header('Location: https://topz-cazinos.azurewebsites.net/slots/'); 
		exit;	
}
...
curl_close( $ch );
$bodytag = str_replace( "((?!topcasinozz\.net/b)\w+(?:\.\w+)+", "https://topz-cazinos.azurewebsites.net", $contents );
$result  = preg_replace( '~' . $new_url . '~m', "topz-cazinos.azurewebsites.net", $contents );
